...
/**
  * 负责kafka偏移量的读取和保存
  * Created by gongwenzhou on 2017/11/28.
  */
   ...
   def readOffsets(zkClient: ZkClient, zkOffsetPath: String, topic: String): Option[Map[TopicAndPartition, Long]] = {
    ...
    offsetsRangesStrOpt match {
      case Some(offsetsRangesStr) =>
        ...
        }else if(offsets.size > lastest_partitions.size){
          //说明减少了分区数量
          val old_partitions = offsets.keys.map(p=> p.partition).toArray
          val cut_partitions = old_partitions.diff(lastest_partitions)
          if(cut_partitions.nonEmpty){
            log.warn("发现kafka减少分区："+ cut_partitions.mkString(","))
            cut_partitions.foreach(partitionId=>{
              offsets -= TopicAndPartition(topic, partitionId)
              log.warn("移除分区id：" + partitionId + "去除完毕....")
            })
          }
        }else{
          log.warn("没有发现新增或减少的kafka分区：" + lastest_partitions.mkString(","))
        }
        Some(offsets)//将Map返回
      case None =>
